...
export interface TextFieldState {
     ...
     */
    renderedControl: HTMLInputElement | null;
}
 ...
 * `<textarea>` validation.
 */
export class InputValidator extends Validator<TextFieldState> {
    private inputControl?: HTMLInputElement;

    protected override computeValidity({ state, renderedControl }: TextFieldState) {
        let input = renderedControl;
        if (!input) {
            // Get cached <input> or create it.
            input = this.inputControl || document.createElement("input");
        ...
        if (inputState.step) {
            input.step = String(inputState.step);
        } else {
    ...
    protected override copy({ state }: TextFieldState): TextFieldState {
        ...
        return {
            state: this.copyInput(state),
            renderedControl: null,
        };
    }

    private copyInput(state: InputState): InputState {
        const { value, required, minlength, maxlength, type, pattern, min, max, step } =
            state;
        return {
            value,
            required,
