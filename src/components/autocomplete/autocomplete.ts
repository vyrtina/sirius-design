...
//TODO: make it possible to add options via html, not just via element property. (ex: <sd-autocomplete><sd-option></sd-option></sd-autocomplete>
//elements added this way are added into options properties upon component creation. they may be then overridden.
...
export default class SdAutocomplete
    ...
    protected get tags() {
        return this.selectedOptions.map((option, index) => {
            if (index < this.maxOptionsVisible || this.maxOptionsVisible <= 0) {
                ...
                // Hit tag limit
                return html`
                    <sd-tag>+${this.selectedOptions.length - index}</sd-tag>`;
            }
            return html``;
        });
    }
    ...
    @property() renderOption: (
        ...
        return html`
            <sd-option value=${option.value}> ${option.label}</sd-option> `;
    };
    ...
    protected renderDisplayInput() {
        ...
        ${this.loading ? html`
            <sd-spinner></sd-spinner>` : nothing}`;
        return this.multiple
            ? html`
                    <div part="tags" class="select__tags">${this.tags}${input}</div>`
            : input;
    }
